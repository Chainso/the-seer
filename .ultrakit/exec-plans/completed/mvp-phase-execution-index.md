...
- `in_progress`: none
- `blocked`: none
- `completed`: `docs/exec-plans/completed/manual-action-retry.md` (archived on 2026-03-17 after adding manual retry for terminal failed actions, exposing a managed-agent run retry button that redirects to the fresh execution, and ratifying the affected lifecycle/spec docs)
- `completed`: `docs/exec-plans/completed/managed-agent-copilot-runtime-and-prompt-split.md` (archived on 2026-03-16 after migrating the Seer-owned managed-agent runner from the prompt-only OpenAI path onto the shared copilot runtime, preserving a separate managed-agent prompt, persisting shared tool transcript activity, and ratifying the affected design/architecture/spec docs)
...
- `completed`: `docs/exec-plans/completed/object-store-model-locked-tabs.md` (archived on 2026-03-13 after turning `/inspector/history` into a model-locked workspace with live-object and embedded-insights tabs, adding model-specific live-object columns with conditional display-name fields, preserving `/inspector/history/object` deep links, and validating with targeted history/insights contract tests plus `cd seer-ui && npm run build`)
- `completed`: `docs/exec-plans/completed/phase-handoff-capsules-in-execution-plans.md` (archived on 2026-03-15 after requiring durable `Phase Handoff` subsections for multi-phase plans in `PLANS.md` and aligning `plan-and-execute` plus `execute-phase` so controller and worker flows both treat the checked-in phase handoff as the authoritative worker brief)
- `completed`: `docs/exec-plans/completed/execution-plan-policy-and-skill-alignment.md` (archived on 2026-03-15 after adding repository-root `PLANS.md`, updating `AGENTS.md` and `docs/exec-plans/README.md` to treat it as the canonical execution-plan contract, and aligning the `plan-and-execute` and `execute-phase` skills to controller-versus-worker responsibilities against one shared policy)
