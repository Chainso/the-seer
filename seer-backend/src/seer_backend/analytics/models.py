...
class ProcessMiningRequest(BaseModel):
    ...
    def validate_time_window(self) -> ProcessMiningRequest:
        ...
        if self.include_object_types and self.anchor_object_type not in self.include_object_types:
            self.include_object_types = [self.anchor_object_type, *self.include_object_types]
        return self
